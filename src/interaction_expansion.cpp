...
K_(buildK(lattice, boost::lexical_cast<unsigned>(parms["NA"]))), 
n_site(2*lattice.num_sites()-NA),
...
beta(1./temperature),  
timestepinv(0.5*n_tau*temperature),
...
M(), 
bare_green_itime(n_tau+1, lattice.num_sites(), K_, beta, timestep),
superbare_green_itime(n_tau+1, n_site, KAB_, KABprime_, beta, timestep),
sweeps(0),
