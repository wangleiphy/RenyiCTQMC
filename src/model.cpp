...
void InteractionExpansion::add()
{
  ...
  double metropolis_weight = (Remove/Add)*(-2.*beta*n_bond*V)/(pert_order+1)* wratios[sector] * exp(lng[sector][pert_order+1]-lng[sector][pert_order])    ; // sector =0 (Z) =1 (W)
...
void InteractionExpansion::remove()
{
    ...
    double metropolis_weight = (Add/Remove)*pert_order/(-2.*beta*n_bond*V)* wratios[sector] * exp(lng[sector][pert_order-1]-lng[sector][pert_order]);
